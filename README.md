...
> **Status:** Shell hosts Surfaces via `mount`/`unmount`. **EPG** is a real Canvas Visible Window Surface (`packages/epg-canvas` + shared math). Home / Live / WebGL Lab still use the stub until Epics 3–5. See [docs/index.md](docs/index.md).  
...
Use arrow keys as D-pad and Enter to **mount**. **EPG** opens the Canvas Program Grid with Visible Window virtualization (HUD shows drawn ≪ logical cells; arrows move the window). Home / Live / WebGL Lab still mount the stub. Back (Backspace / Escape) **unmounts** and returns to the menu. Story **2.2–2.4** add full EPG focus detail, now-line, and Perf Note; Story **6.2** Memory Soak validates cleanup across real Surfaces.
